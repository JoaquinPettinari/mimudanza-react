...
function WhatsAppButton({ title, id }: WhatsAppButtonProps) {
  function makeWhatsAppLink(mensaje: string) {
    return `https://wa.me/541161511893?text=${encodeURIComponent(mensaje)}`;
  }
  ...
  return (
    <a href={linkToWhatsApp} className="w-full md:w-2/3">
      <button className="flex items-center gap-3 bg-[#25d466] rounded-full p-4 w-full justify-center">
        ...
        <h3 className="text-xl text-white font-semibold font-[Outfit,sans-serif!important]">
          ¡Quiero saber mas!
        </h3>
      </button>
    </a>
